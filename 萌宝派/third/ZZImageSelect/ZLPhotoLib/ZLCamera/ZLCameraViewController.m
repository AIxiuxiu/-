...
- (void) setup{
    ...
    // 头部View
    UIButton *deviceBtn = [self setupButtonWithImageName:@"PhotoResource.bundle/xiang" andX:self.view.width - margin - width];
    [deviceBtn addTarget:self action:@selector(changeCameraDevice:) forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *flashBtn = [self setupButtonWithImageName:@"PhotoResource.bundle/shanguangdeng" andX:10];
    [flashBtn addTarget:self action:@selector(flashCameraDevice:) forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *closeBtn = [self setupButtonWithImageName:@"PhotoResource.bundle/shanguangdeng2" andX:60];
    ...
    [cameraBtn setImage:[UIImage imageNamed:@"PhotoResource.bundle/paizhao"] forState:UIControlStateNormal];
...
-(void)takePhoto
{
        ...
        if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera])
        {
            ...
            @catch (NSException *exception) {
                [[[[UIApplication sharedApplication].windows lastObject] rootViewController] presentViewController:camreaVc animated:YES completion:nil];
            }
