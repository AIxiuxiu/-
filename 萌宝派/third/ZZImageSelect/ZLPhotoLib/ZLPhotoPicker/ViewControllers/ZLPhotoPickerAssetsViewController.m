...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        rightBtn.frame = CGRectMake(0, 0, 70, 45);
...
- (UICollectionView *)toolBarThumbCollectionView{
    if (!_toolBarThumbCollectionView) {
        ...
        UICollectionView *toolBarThumbCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(10, 0, self.view.width - 118, 44) collectionViewLayout:flowLayout];
...
//    self.makeView.text = [NSString stringWithFormat:@"%ld",(long)count];
    ...
    [self  updateDoneButtonContent:count];
}
...
-(void)setMinCount:(NSInteger)minCount{
    ...
    if (self.selectAssets.count == minCount){
        ...
        [self  updateDoneButtonContent:self.selectAssets.count];
...
#warning self write  加上图片选择数
-(void)updateDoneButtonContent:(NSUInteger)count{
    NSString  *tip = [NSString  stringWithFormat:@"%ld/%ld 完成",count,self.minCount];
    [self.doneBtn  setTitle:tip forState:UIControlStateNormal];
}
@end
