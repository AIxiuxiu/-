...
+ (void)initialize
{
    ...
    // 2.创表
    [_db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_RongUserInfo (id INTEGER PRIMARY KEY, userId  TEXT NOT NULL UNIQUE, name TEXT , portraitUri TEXT);"];
}
+(void)uploadUserInfoWithUserId:(NSString*)userid  andNick:(NSString*)nick  andImageurl:(NSString*)imageurl{
    ...
    if ([self  selectDataWithRongUserInfoUserId:userid]) {
    [self  updateRongUserInfo:userInfo ]  ;
    }else{
               [self  addRongUserInfo:userInfo]   ;
